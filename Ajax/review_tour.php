...
if(isset($_POST['review_form']))
{
    ...
    $upd_query = "UPDATE `booking_order1` SET `rate_review`=? WHERE `booking_id` = ? AND `user_id`=?";
    $upd_values = [1,$frm_data['booking_id'],$_SESSION['uId']];
    $upd_result = update($upd_query,$upd_values,'iii');


    $ins_query = "INSERT INTO `rating_review1`(`booking_id`, `tour_id`, `user_id`, `rating`, `review`) VALUES (?,?,?,?,?)";
    $ins_values = [$frm_data['booking_id'],$frm_data['tour_id'],$_SESSION['uId'],
