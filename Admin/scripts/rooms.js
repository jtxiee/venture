...
function add_room() {
    ...
    xhr.onload = function() {
        ...
        if (this.responseText == '1') {
            ...
            add_room_form.reset();
            get_all_rooms();
        } else if (this.responseText == 'Room exists') {
            alert('error', 'Room is already available!');
        } else {
...
function submit_edit_room() {
    ...
    //     }
    // });
    let featureCheckboxes = edit_room_form.elements['features'];
    for (let i = 0; i < featureCheckboxes.length; i++) {
        let el = featureCheckboxes[i];
        if (el.checked) {
            features.push(el.value);
        }
    }
    ...
    //     }
    // });
    let facilityCheckboxes = edit_room_form.elements['facilities'];
    for (let i = 0; i < facilityCheckboxes.length; i++) {
        let el = facilityCheckboxes[i];
        if (el.checked) {
            facilities.push(el.value);
        }
    }
