...
if (isset($_POST['add_room'])) {
    ...
    // Kiểm tra xem một phòng với cùng tên đã tồn tại hay chưa
    $check_query = "SELECT COUNT(*) FROM `rooms` WHERE `name` = ?";
    if ($stmt = mysqli_prepare($con, $check_query)) {
        mysqli_stmt_bind_param($stmt, 's', $frm_data['name']);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $room_count);
        mysqli_stmt_fetch($stmt);
        mysqli_stmt_close($stmt);

        if ($room_count > 0) {
            // Phòng với cùng tên đã tồn tại
            echo "Room exists";
            exit;
        }
    } else {
        die('Không thể chuẩn bị truy vấn - kiểm tra');
    }

    // Tiến hành chèn nếu không tìm thấy phòng trùng
    ...
    if ($stmt = mysqli_prepare($con, $q2)) {
        ...
        $flag = 0;
        die('Không thể chuẩn bị truy vấn - chèn');
    }
...
if (isset($_POST['rem_image'])) {
    ...
    $value = [$frm_data['image_id'],$frm_data['room_id']];
